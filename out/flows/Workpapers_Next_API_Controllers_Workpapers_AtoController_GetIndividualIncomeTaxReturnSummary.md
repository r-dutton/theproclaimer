[web] GET /api/ato/individual-income-tax-returns/summary  (Workpapers.Next.API.Controllers.Workpapers.AtoController.GetIndividualIncomeTaxReturnSummary)  [L82–L88] status=200 [auth=AuthorizationPolicies.User]
  ...
  └─ impact_summary
    └─ remote_endpoints 1 (calls=1) scopes=service:1
      └─ GET /api/gov-link/individual-income-tax-returns/summary -> DataGet.Api via DataGetClient [query] (x1)
    └─ clients 1
      └─ DataGetClient
    └─ services 1
      └─ AtoService
    └─ requests 1
      └─ GetIndividualIncomeTaxReturnSummaryQuery
    └─ handlers 1
      └─ GetIndividualIncomeTaxReturnSummaryQueryHandler
