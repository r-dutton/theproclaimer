[web] PUT /api/accounting/reports/templates/{id}/reorder  (Cirrus.Api.Controllers.Accounting.Reports.ReportTemplatesController.Reorder)  [L312–L330] status=200 [auth=user]
  └─ calls ReportTemplateRepository (methods: WriteQuery,ReadQuery) [L321]
  └─ write ReportTemplate [L321]
  └─ query ReportTemplate [L315]
  └─ sends_request CanIAccessFileQuery -> CanIAccessFileQueryHandler [L319]
  ...
  └─ impact_summary
    └─ entities 1 (writes=1, reads=1)
      └─ ReportTemplate writes=1 reads=1
    └─ requests 1
      └─ CanIAccessFileQuery
    └─ handlers 1
