[web] POST /api/accounting/assets/depreciation-years/  (Cirrus.Api.Controllers.Accounting.Assets.DepreciationYearController.Create)  [L72–L87] status=201 [auth=user]
  └─ calls DepreciationYearRepository (methods: Add,ReadQuery) [L84]
  └─ insert DepreciationYear [L84]
    └─ reads_from DepreciationYears
  └─ query DepreciationYear [L77]
    └─ reads_from DepreciationYears
  └─ sends_request CanIAccessFileQuery -> CanIAccessFileQueryHandler [L75]
  ...
  └─ impact_summary
    └─ entities 1 (writes=1, reads=1)
      └─ DepreciationYear writes=1 reads=1
    └─ requests 1
      └─ CanIAccessFileQuery
    └─ handlers 1
